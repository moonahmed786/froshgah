...
class BlogController extends Controller
{
    ...
    public function store_new_blog(Request $request){
        ...
        $blog = Blog::where("slug",$slug)->count();
        $final_slug = $blog < 1 ? $slug : $slug . '-' . rand(111111,999999);
    ...
    public function clone_blog(Request $request)
    {
        ...
        $slug = !empty($blog_details->slug) ? \Str::slug($blog_details->slug) . '-' . rand(1000,9999) : \Str::slug($blog_details->title) ;
    ...
    public function update_blog(Request $request,$id){
        ...
        if($blog->count() > 0){
            $final_slug = $blog->id == $id ? $slug : $slug . '-' . rand(111111,999999);
        }else{
            $final_slug = $slug;
        }
